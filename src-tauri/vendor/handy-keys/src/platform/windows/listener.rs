...
fn spawn_inner(
    blocking_hotkeys: Option<BlockingHotkeys>,
    #[cfg(test)] mut before_install: impl FnMut(HookStage) -> windows::core::Result<()> + Send + 'static,
) -> Result<WindowsListenerState> {
    let (tx, rx) = mpsc::channel();
    let (ready, readiness) = mpsc::channel::<Result<()>>();
    ...
    let handle = thread::Builder::new()
        .spawn(move || {
            ...
            #[cfg(test)]
            let keyboard_setup = before_install(HookStage::Keyboard);
            #[cfg(not(test))]
            let keyboard_setup: windows::core::Result<()> = Ok(());
            let kb_hook = keyboard_setup.and_then(|()| unsafe {
            ...
            let mut kb_hook = match kb_hook {
                Ok(h) => h,
                Err(e) => {
                    let _ = ready.send(Err(crate::Error::Platform(format!(
            ...
            #[cfg(test)]
            let mouse_setup = before_install(HookStage::Mouse);
            #[cfg(not(test))]
            let mouse_setup: windows::core::Result<()> = Ok(());
            let mouse_hook = mouse_setup.and_then(|()| unsafe {
            ...
            // The constructor must not expose a listener until both native hooks
            // exist and their owning thread is about to service the message queue.
            // If the caller disappeared, use the normal cleanup path below.
    ...
    match readiness.recv() {
        ...
        outcome => {
            ...
            // In particular, finish keyboard cleanup after a mouse-hook failure.
            let _ = handle.join();
            return Err(match outcome {
                Ok(Err(error)) => error,
                _ => crate::Error::Platform(
                    "Windows hook thread stopped before reporting readiness".into(),
                ),
            });
        }
    }
